...
/*   Updated: 2023/12/04 17:26:13 by aurban           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	pipex_fork_child(t_sdata *sd, char **envp)
{
	...
	close(sd->daddy[0]);
	close(sd->daddy[1]);
	if (execve(sd->cmds_paths[i - 1], sd->cmds_files[i], envp) == -1)
	{
		free(sd->cmds_paths[i - 1]);
		sd->cmds_paths[i - 1] = NULL;
...
int	pipex_start(char ***cmds_files, int argc, char **envp)
{
	...
	cmds_paths = get_commands_path(argc, cmds_files, envp);
	...
	clean_cmd_paths(cmds_paths, argc - 2);
	free(buffer);
	return (0);
}
